...
const createUniformSetters = (
  ...
  const createUniformSetter = (info: WebGLActiveInfo): ((x: any) => void) => {
    ...
    switch (type) {
      case gl.FLOAT:
        if (isArray) return (v) => gl.uniform1fv(location, v);
        else return (v) => gl.uniform1f(location, v);
      case gl.FLOAT_VEC2:
      ...
      case gl.INT:
        if (isArray) return (v) => gl.uniform1iv(location, v);
        else return (v) => gl.uniform1i(location, v);
      case gl.INT_VEC2:
...
export const createBufferInfoFromArrays = (
  ...
  if (
    attribArrays.a_position.data.length %
      attribArrays.a_position.numComponents !==
    0
  ) {
